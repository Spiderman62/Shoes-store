...
const handle_quantity = {
	handle_event(){
		const minus = document.querySelectorAll('.cart_component .quantity .minus');
		const plus = document.querySelectorAll('.cart_component .quantity .plus');
		const quantity = document.querySelectorAll('.cart_component .quantity input');
		const ammount = document.querySelectorAll('.cart_component .ammount');
		
		plus.forEach((item,index)=>{
			item.addEventListener('click',e=>{
				if(parseInt(quantity[index].value) < 10){
					quantity[index].value = parseInt(quantity[index].value) + 1; 
					// ammount[index].innerText
				}else{
					alert('sản phẩm chỉ chứa tối đa 10 số lượng');
				}
			});
		...
		minus.forEach((item,index)=>{
			item.addEventListener('click',e=>{
				if(parseInt(quantity[index].value) > 0){
					quantity[index].value = parseInt(quantity[index].value) - 1; 
				}else{
					alert('Không có sản phẩm được chỉ định');
				}	
			});
